...
const Edit = ({questions, question, upForm, setUpForm, setQuestions}) => {
    ...
    const [valueDescriptionUpdate, setValueDescriptionUpdate] = useState(`${description}`);

    const updateQuestion = async (title, description) => {
        try {
            const { data } = await axios.put(`http://localhost:3001/api/anat/${id}`, {
                title,
                description,
            })

            const newQuestion = {
                title: valueTitleUpdate, description: valueDescriptionUpdate
            };
